...
- BOTONES DE STREAMLIT OCULTOS (Share, Edit, GitHub, Menú)
IMPORTANTE:
...
if st.session_state.get('demo_mode', False):
    with st.sidebar:
        ...
        if st.button( "💳 Actualizar a Premium ", key= "upgrade_from_demo "):
            ...
            st.rerun()
        logout()
    return  # Salimos de la función si n bloquear
...
if st.session_state.get('payment_intent', False):
    ...
    if 'payment' in query_params and query_params['payment'] == 'success' and 'collection_id' in query_params:
        payment_id = query _params['collection_id']
        if check_payment_status(payment_id):
            ...
            st.session_state.demo_m ode = False
...
rad_diaria = 20 + 5 * np.sin(np.linspace(0, 4np.pi, dias)) + np.random.randn(dias)3
wind_diaria = 3 + 2 * np.sin(np.linspace(0, 2np.pi, dias)) + np.random.randn(dias)*1
return {
    'precipitacion': {
...
polygons = []
coord_sections = re.findall(r'<coordinates[^>]>([\s\S]?)', content, re.IGNORECASE)
...
with z ipfile.ZipFile(io.BytesIO(file_content), 'r') as zip_ref:
...
gdf = procesar_kml_robusto(file _content)
if gdf is None or len(gdf) == 0:
...
elif geom etria_unida.geom_type == 'MultiPolygon':
poligonos = list(geometria_unida.geoms)
poligonos.sort(key=lambda p: p.area, reverse=True)
gdf_unido = gpd.GeoDataFrame([{'geometry': polig onos[0]}], crs='EPSG:4326')
else:
...
try:
    ...
    os.unlink(download_p ath)  # eliminar archivo temporal
...
lat = centroide.y
lon = centroide.x
start = fecha_inicio.strfti me( "%Y%m%d ")
...
radiacion = props.get('ALLSKY _SFC_SW_DWN', {})
...
if 'radiacion' in datos_climaticos and datos_climaticos['radiacion'].get('diaria'):
    ...
    if 'promedio' in datos_ climaticos['radiacion']:
...
# Temperatura
if 'temperatura' in datos_climaticos and datos_climaticos['temperatura'].get('diar ia'):
    temp = np.array(datos_climaticos['temperatura']['diaria'][:n_dias], dtype=np.float64)
    mask_nan = np.isnan(temp)
    if np.any(mask_nan):
        temp_filled = temp.co py()
        temp_filled[mask_nan] = np.nanmean(temp)
    else:
        temp_filled = temp
    ax4 = axes[1, 1]
    ax4.plot(dias, temp_filled, '^-', color='red', linewidth=2, mark ersize=4)
...
alt_base = 'Arcilloso Pesado'
    ...
    for idx, row in gdf_dividido.iterrows():
        centroid = row.geometry.centr oid
...
maxy = bounds[3] + 0.1 * dy
    ...
    cmap = matplotlib.colors.LinearSegmentedColormap.from_list('cu stom', colormap_list)
    ...
    folium.GeoJson(
        gpd.GeoS eries(plantacion_union).to_json(),
    ...
    folium.LayerControl(co llapsed=False).add_to(m)
    Fullscreen().add_to(m)
...
if mapa_calor:
    ...
    ax.set_t itle(f'Valores de {titulo} por bloque')
    st.pyplot(fig)
    plt.close(fig)
...
fig.update_traces(marker=dict(line=dict(width=1, color='D arkSlateGrey')))
...
with col1:
    ...
    st.dataframe(top_ndvi.style.format({'NDV I': '{:.3f}'}), use_container_width=True)
    ...
    st.dataframe(bottom_ndvi.style.fo rmat({'NDVI': '{:.3f}'}), use_container_width=True)

with col2:
    st.markdown( "#### Top 5 NDWI ")
    top_ndwi = df.nlargest(5, 'ndwi_modis')[['id_bloque', 'ndwi_modis', 'salud']]
    top_ndwi.columns = ['Bloque', 'NDWI', 'Salud']
    st.dataframe(top_ndwi.style.format({'NDW I': '{:.3f}'}), use_container_width=True)
    ...
    st.dataframe(bottom_ndwi.style.fo rmat({'NDWI': '{:.3f}'}), use_container_width=True)
...
gdf = st.session_state.gdf_original.copy()
    if st.session_state.demo_mode:
        ...
        gdf_dividido = dividir_plantacion_en_bloques(gdf, n _divisiones)
        areas_ha = []
        for idx, row in gdf_dividido.iterrows():
            area_gdf = gpd.GeoDataFrame({'geometry': [row.geometry]}, crs=gdf_dividido.crs)
             areas_ha.append(float(calcular_superficie(area_gdf)))
        ...
        st.session_state.datos_modis = {
            'ndvi': gdf _dividido['ndvi_modis'].mean(),
            'ndwi': gdf_dividido['ndwi_modis'].mean(),
            'fecha': fecha_inicio.strftime('%Y-%m-%d'),
    ...
    st.session_state.anali sis_completado = True
...
<div class="hero-banner">
    ...
    <p style="color: #a5d6a7; font-size: 1.1em;">Monitoreo biológico con datos reales NASA Earthdata · Open-Meteo · NASA POWER</p>
</div>
""", unsafe_allow_html=True)
===== SIDEBAR =====
with st.sidebar:
...
if hasattr(fecha_fin, 'year'): fecha_fin = dateti me.combine(fecha_fin, datetime.min.time())
except: pass
...
if gdf_completo is not None:
    ...
    with tab1:
        ...
        ndwi_prom = gdf_completo['ndwi_modis'].mean() if 'ndwi_modis' in gdf_completo.column s else np.nan
        ...
        pct_ buena = (salud_counts.get('Buena', 0) / total_bloques * 100) if total_bloques  > 0 else 0
        ...
        with col_g1:
            ...
            if not salud_counts.empty:
                ...
                colors_pie = {'Crítica': '#d73027', 'Baja': '#fee08b', 'Mode rada': '#91cf60', 'Buena': '#1a9850'}
        ...
        with col_g2:
            ...
            if 'ndvi_modis' in gdf_completo.columns and 'edad_anios' in gdf_completo.columns:
                ...
                ax_ hist.hist(gdf_completo['ndvi_modis'].dropna(), bins=15, alpha=0.7, label='NDVI', color='green')
                ax_hist.set_xlabel('NDVI')
                ax_hist.set_ylabel('Frecu encia', color='green')
                ax_hist.tick_params(axis='y', labelcolor='green')
                
                ax2 = ax_hist.twinx()
                ax2.hist(gdf_complet o['edad_anios'].dropna(), bins=15, alpha=0.5, label='Edad', color='orange')
            ...
            if st.session_state.palmas_detectadas:
                ...
                for i, palma in enumerate(st.session_state.palmas_detectadas[:2000]):
                    if 'centroide' in palma:
                        lon, lat = palma['cent roide']
                        folium.CircleMarker([lat, lon], radius=2, color='red', fill=True,
                                            fill_color='red', fill_opacity=0.8).ad d_to(palmas_group)
    ...
    with tab5:
        ...
        if st.session_state.deteccion_ejecutada and st.session_state.palmas_detectadas:
            ...
            try:
                ...
                for i, palma in enume rate(palmas[:2000]):
                    if 'centroide' in palma:
                        lon, lat = palma['centroide']
                        folium.CircleMarker([lat, lon], radi us=2, color='red', fill=True, 
            ...
            if palmas:
                try:
                    ...
                    geojson_pal mas = gdf_palmas.to_json(); csv_palmas = df_palmas.to_csv(index=False)
    ...
    with tab6:
        ...
        if datos_fertilidad:
            ...
            col1, col2, col3, col4, col5 = st.columns(5)
            with col1: N_prom = df_fer tilidad['N_kg_ha'].mean(); st.metric( "Nitrógeno (N) ", f "{N_prom:.0f} kg/ha ")
            ...
            variable = st.selectbox(
                ...
                format_func=lambda x: {
                    ...
                    'pH': 'pH del suelo',
                    'MO_porcen taje': 'Materia Orgánica (%)'
                }[x]
            )
            ...
            df_recom = df_fertilidad[['id_bloque', 'N_kg_ha', 'P_kg_ha', 'K_kg_ha', 'pH', 
                                      'recomendacion_N', 'recomendacion_P', 'recomenda cion_K']].copy()
    ...
    with tab7:
        ...
        if textura_por_bloque:
            ...
            try:
                ...
                tipos_unicos = gdf_textura['tipo_suelo'].uniqu e()
                colores = ['#8B4513', '#D2691E', '#F4A460', '#DEB887', '#BC8F8F', '#CD853F']
                color_dict = {tipo: colores[i % len(colores)] for i, tipo in enumer ate(tipos_unicos)}
                ...
                folium.GeoJson(
                    ...
                    tooltip=folium.GeoJsonToolt ip(fields=['id_bloque','tipo_suelo','arena','limo','arcilla','drenaje'],
                                                  aliases=['Bloque','Tipo','Arena %','Limo %','Arcilla %',' Drenaje'])
                ).add_to(m_textura)
                folium.LayerControl().add_to(m_textura); Fullscreen().add_to(m_textura)
                folium_static(m_textura, widt h=1000, height=600)
            except Exception as e:
            ...
            ax.bar(df_plot['id_bloq ue'].astype(str), df_plot['arcilla'], 
                   bottom=df_plot['arena']+df_plot['limo'], label='Arcilla', color='#8B4513')
            ax.set_xlabel('Bloque'); ax.set_yla bel('Porcentaje')
            ax.set_title('Composición Textural por Bloque'); ax.legend()
            plt.xticks(rotation=45); plt.tight_layout()
            st.pyplot(fig); plt.c lose(fig)
            ...
            if len(df_textura)  > 0:
                ...
                st.plot ly_chart(fig_tri, use_container_width=True)
            ...
            with st.spinner( "Procesando DEM y generando isolíneas... "):
                ...
                if gdf_original is None:
                    ...
                    if not st.session_state.demo_mode and api_key:
                        ...
                        curvas = generar_curvas_nivel_simuladas(gdf_o riginal)
                        st.info(f "ℹ️ Usando relieve simulado. Se generaron {len(curvas)} curvas de nivel. ")
                    
                    if curvas:
                        st.session_state.curvas_nivel = curvas
                        m_curvas = mapa_curvas_coloreadas(gdf_ original, curvas)
                        folium_static(m_curvas, width=1000, height=600)
                        gdf_curvas = gpd.GeoDataFrame(
                            {'eleva cion': [e for _, e in curvas], 'geometry': [l for l, _ in curvas]},
                            crs='EPSG:4326'
                        )
                        geojson_curvas = g df_curvas.to_json()
            ...
            try:
             ...
             if not YOLO_AVAILABLE:
                ...
                if archivo_imagen is not None and archivo_modelo is not None:
                    with tempfile.NamedTempor aryFile(delete=False, suffix=os.path.splitext(archivo_modelo.name)[1]) as tmp_model:
                        tmp_model.write(archivo_modelo.read())
                        ruta_mod elo_tmp = tmp_model.name

                    imagen_bytes = archivo_imagen.read()
                    imagen_pil = Image.open(io.BytesIO(imagen_bytes))
                    imagen_ cv = cv2.cvtColor(np.array(imagen_pil), cv2.COLOR_RGB2BGR)
                    ...
                    if modelo is not None:
                        ...
                        if resultados_yolo and len(resultados_y olo)  > 0:
                            ...
                            st.markdown(leyenda_html, unsafe_allow_html=Tru e)
                            ...
                            csv_detec ciones = df_detecciones.to_csv(index=False)
